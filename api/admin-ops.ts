    ...
    switch (action) {
      ...
      case 'bulk_reconcile': {
        ...
        if (staleTxs && staleTxs.length > 0) {
          console.log(`🧹 [Admin Ops] Cleaning up ${staleTxs.length} stale initialized transactions...`);
          const staleIds = staleTxs.map(tx => tx.id);
          await supabase
            .from("transactions")
            .update({ 
              status: "failed", 
              ...
              vtu_status: "failed",
              error_message: "Abandoned: Initialized for > 1 hour without payment.",
              updated_at: new Date().toISOString()
            })
            .in("id", staleIds);
        }
