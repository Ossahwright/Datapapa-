...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const endpoints = [
      "https://datahubgh.com/api/balance",
      `${baseUrl}/user`,
      `${baseUrl}/balance`,
      `${parentUrl}/user`,
      `${baseUrl}/fetch-user`,
      ...
      try {
        ...
        if (resp.status === 200 && resp.data) {
          ...
          return res.json({ balance, url }); 
           ...
           errors.push(`[Bearer] ${url} -> ${resp.status}`);
        }
      } catch (err: any) {
         errors.push(`[Bearer] ${url} -> ${err.message}`);
      }
      ...
      try {
        ...
        if (resp.status === 200 && resp.data) {
          ...
          return res.json({ balance, url }); 
           ...
           errors.push(`[X-API-Key] ${url} -> ${resp.status}`);
        }
      } catch (err: any) {
         errors.push(`[X-API-Key] ${url} -> ${err.message}`);
      }
    }
    
    return res.status(200).json({ balance: 0, error: `Could not fetch balance. Attempts: ${errors.join(', ')}` });
  } catch (err: any) {
