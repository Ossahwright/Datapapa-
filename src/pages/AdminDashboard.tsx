...
export default function AdminDashboard() {
  ...
  const handleWhatsAppReceipt = async (tx: any) => {
    ...
    console.log("[Receipt Capture] Preparing to capture receipt for transaction:", tx.id);
    await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    await new Promise((resolve) => setTimeout(resolve, 150));

    const element = document.getElementById(`receipt-capture-${tx.id}`);
    if (!element) {
      console.error("[Receipt Capture] Failed to discover element with ID:", `receipt-capture-${tx.id}`);
    ...
    console.log("[Receipt Capture] Element found, starting screenshot generation...", element.id);
    ...
    for (const sheetEl of linkPlusStyles) {
      try {
        ...
        console.warn("[Receipt Capture] Detaching cross-origin stylesheet before screenshot:", (sheetEl as any).href || "inline style tag");
        detachedSheets.push({
          element: sheetEl,
    ...
    try {
      console.log("[Receipt Capture] Running html-to-image toJpeg render stage...");
      ...
      console.log("[Receipt Capture] JPEG generation successful. Preparing file download...");
      ...
      console.log(`[Receipt Capture] Screenshot successfully downloaded as receipt-${identifier}.jpeg`);
      toast.success("Receipt downloaded! Opening WhatsApp chat...");
    } catch (err) {
      console.error("[Receipt Capture] toJpeg fail error:", err);
      ...
      for (const { element: el, parent, nextSibling } of detachedSheets) {
        if (parent) {
          try {
            ...
            console.error("[Receipt Capture] Error while restoring stylesheet:", restoreErr);
          }
        }
      }
      setCapturingReceiptTx(null);
              ...
              <div className="w-full mt-6 bg-slate-50 rounded-2xl p-5 border border-slate-200 relative text-left">
                ...
                <div className="space-y-3.5 text-xs text-slate-600 font-mono">
                  ...
                  <div className="flex justify-between font-bold text-slate-600">
                    <span className="uppercase text-[9px] font-semibold text-slate-400 font-mono">Payer No.</span>
                  ...
                  <div className="flex justify-between font-bold text-slate-600">
                    <span className="uppercase text-[9px] font-semibold text-slate-400 font-mono">Date</span>
