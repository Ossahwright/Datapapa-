...
const NETWORKS = [
  { id: 'mtn', name: 'MTN', color: 'bg-yellow-500', text: 'text-yellow-950', logo: 'https://i.postimg.cc/5NPHHMBJ/MTN-LOGO-1.png' },
  { id: 'telecel', name: 'Telecel', color: 'bg-red-600', text: 'text-white', logo: 'https://i.postimg.cc/SRgWNYSf/TELECEL-LOGO-1.jpg' },
  { id: 'airteltigo', name: 'AirtelTigo', color: 'bg-red-500', text: 'text-white', logo: 'https://i.postimg.cc/0yXPdkQf/AIRTELTIGO-LOGO-1.jpg' },
];
...
export default function BuyDataForm() {
  ...
  const allBundles = dbBundles
    .filter(b => b.is_active !== false) // Only show active bundles
    .reduce((acc: any, b: any) => {
      let net = (b.network_key || b.network || '').toLowerCase();
      if (net === 'at') net = 'airteltigo'; // normalize
      if (net === 'vodafone') net = 'telecel'; // normalize
      if (!acc[net]) acc[net] = [];
      acc[net].push({ 
        id: b.id, 
        name: b.description || '', 
        price: Number(b.selling_price || b.price || 0), 
